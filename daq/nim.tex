...
\begin{figure*}[htbp]
	\centering
	\includegraphics[width=0.7\textwidth]{./koala_setup.png}
...
recoil measurement technique was verified using proton beams \cite{recoil_article}.
...
In order to suppress them and achieve the required $|t|$ range, a forward detector based on fast-timing plastic scintillator is
...
All components inside these arms are installed in the same vacuum space as the
proton beam.
...
It is $\sim\SI{1.5}{\mm}$ close to the interaction point (IP).
The areal density is estimated to be \SI{e14}{\atom\per\cm\squared}.

\subsection{Recoil detector}
\label{sec:recoil}

The recoil detector needs to stop the recoil proton from the
...
Sensors have different thickness: \SI{1}{\mm} for Si1
...
Soild-state sensors (especially germanium) need low and stable operating
...
\begin{figure}[htb]
  ...
  \caption{
    ...
    $pp$ elasitc scattering: a) ideal point beam; b) $\SI{10}{\mm}\times\SI{10}{\mm}$ box beam profile.
    }
  \label{fig:forward_acceptance}
\end{figure}
Based on the simulation study, the current width of the forward detector scintillator guarantees the
...
for the charge measurement directly.
Modules are tested in the labrotary using the cosmic ray before installation.
...
Support for the VME controller (SIS3100 \cite{sis}) is integrated and a new component of online monitoring based is added.
