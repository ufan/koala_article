...
The wide peaks around \num{1200} counts on the QDC spectrum are induced by the beam particles
...
A further study of the response are carried out using the selected sample of elastic scattering events.
The selection criteria of the elastic scattering events are described in more detail in Sec. \ref{sec:elastic_extraction}.
...
The low energy thresholds of elastic peaks are clearly seen around \num{1000}.
They are well separated from the noise pedestals, with the signal to noise ratios
...
The detection efficiencies of the forward modules can also be determined with the same elastic scattering sample events, which are about \SI{99.9978}{\percent} and \SI{99.9560}{\percent} respectively.
...
Energy spectrums of the recoil detector are summarized in Fig. \ref{fig:e_map}.
\begin{figure}[htb!]
  \centering
...
The elastic peaks are well separated from the low energy backgrounds for the
channels at large recoil angles, while they are hard to be identified at small
recoil angles.

% Elastic selection based on TOF-E
For the channels which are covered by the forward detector, the correlation
between the recoil proton and the scattering proton is utilized to identify
the elastic events.
...
The spectrum has been corrected with the time-walk parameters, thus the
horizontal band matches the time-walk effect.
This band comes from the elastic events hitting the edge of the recoil sensors.
The upper band fullfills the correct TOF-E relation of the recoil proton and it comes
from the residual target gas distribution downstream the target center.
This is in accordance with the fact that a vacuum pump is installed close to the
target chamber upstream the target center, while none is installed between the target chamber and the forward chamber.

% selection of elastic events and the recoil sigma distribution
The selection of the elastic events is carried out on the TOF-E spectrum as follows:
...
3) the obtained TOF-E relation combining with the $[-5\sigma_{TOF}, 5\sigma_{TOF}]$
window is finally applied to filter out the elastic events.
...
% Elastic energy spectrum and Fwd-fully covered area explaination.
