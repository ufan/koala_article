...
calibration (about \SI{20}{\keV} in terms of FWHM at \SI{5}{\MeV}) is much smaller than the extracted width of the elastic peak from the beam commissioning data (about \SI{330}{\keV} at the equivalent energy as the $\alpha$ source).
...
the MIP peak ($\lesssim$ \SI{1}{\MeV}), and both peaks are embeded among the large low-energy background.
In this case, it's hard to estimate the parameters of the background components
through sideband fit, and there is large error in determining the fraction of
each component.
A pre-selection of the elastic scattering events is needed to get a cleaner spectrum for the fit.
