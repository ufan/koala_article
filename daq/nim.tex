...
\begin{frontmatter}	
  ...
  % electronics group
  \author{Peter Wüstner}

  % other collaborators
  \author{Dieter Grzonka}
  \author{Frank Goldenbaum}
  \author{Susan Schadmand}
  ...
  \begin{abstract}
    ...
    A forward detector is also added into the setup to measure the coincident scattering beam particle to
    ...
    The preliminary results verified that a desired range of |t| could be achieved with the help of
...
over a large 4-momentum transfer range \(0.0008 < |t| < 0.1\) \((GeV/c)^2\).
...
To reach the region \(0.0008 < |t| < 0.004\) \((GeV/c)^2\), a coincidence measurement between the recoil proton and scattering beam particle is needed to suppress the background contamination.
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{./recoil_schematic.png}
...
The forward detector aims to measure the arrival time of the scattering proton over the region \(0.0008 < |t| < 0.004\) \((GeV/c)^2\).
...
Each forward detector module consists of one piece of square BC-408 with dimension \(90 (length) \times 20 (width) \times 6 (thickness)\) mm$^3$.
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.7\textwidth]{./fwd_acceptance.png}
\caption{Simultion result of the distribution of the scattering pronton on
  the forward detector plane at beam momentum 2.6 GeV/c for the elastic scattering events in which
  ...
  The red square shows the size of the forward
  scintillator. This indicates the acceptance of the forward scintillator.}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{./forward_mip.pdf}
...
\begin{figure*}[htbp]
\centering
\includegraphics[width=0.8\textwidth]{./trigger_logic.png}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.5\textwidth]{./linearity.png}
...
\end{enumerate}
The fitting parameters of the last step are the parameters used to convert ADC values into energy values in reconstruction.
...
These offset values are used to align the timestamps from different channels in the reconstruction.
