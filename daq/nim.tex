...
For the channels of the recoil detector at large recoil angles, the elastic
peaks are well separated from the high portion of low energy backgrouds.
It's possible to extract the elastic peak using a combined fit of the spectrum with an appropriate model of the
background for these channels.
...
of the recoil channel and the area under the fitted elastic peak is extracted as
the event counts of the elastic scattering.

\subsection{Extraction of elastic scattering events at small recoil angles}
At small recoil angles, the accuracy of the extraction method, because the elastic peak:
1) overlaps with the MIP peak; 2) is supressed by the large amouts of low energy backgrounds.
Thus, a pre-selection of the elastic scattering events is needed.
For the channels which are covered by the forward detector, the correlation
between the recoil proton and the scattering proton is utilized.
...
Most background events are supressed by requiring that both
...
Fig. \ref{fig:cut} shows the results of this selection from two channels located
at recoil angle .
\begin{figure}[h!]
  \centering
