...
Since there is no tracking device in KOALA, this spread will deteriorate the
...
Typically COSY stores $10^{10}$ protons and the beam has a revolution frq of about
10^6, thus
% The typical proton beam intensity of COSY is about
% \SI{e16}{\atom\per\second},
giving an estimated luminosity of $L
\approx 10^{30}\si{cm^{-2}s^{-1}}$
...
Due to the strong variation of the recoil particle's kinetic energy with angle,
the optimal detector's thickness varies with the .
Since commercially-avalibale silicon sensors are not thick enough to measure the deposit energy up to \SI{60}{\MeV},
thick germanium sensors were developed and manufactured at IKP.
...
Due to the large variation of kinetic energy with recoil angle, the setup
consists of sensors of different material and thickness.
The setup consists of two single-sided silicon strip sensors of same thickness at small recoil
angles and two single-sided germanium strip sensors of the differenet thickness at large recoil angles.
...
sensor strips are approximately \SI{20}{\keV} and \SI{30}{\keV} (FWHM) at despoited
...
\label{sec:daq}
The event rate of $pp$ elastic scattering at COSY is in the order of $10^2$ \si{\per\second}.
%%%% TODO HESR rate
HESR a factor of 2 lower.
...
%%%%% TODO reomve the other option and answer the reviewer about the logic and reason
...
the cluster is also critical in the KOALA experiment, so as to correctly reconstruct the recoil angle of the event.
...
peak, which exists in the spectrum before clustering, correctly migrates into
the elastic peak.
The quality of the energy spectrum in terms of peak-to-valley ratio increases from about 33 to
about 234, which corresponds to a 7-fold improvement.
...
these tests.
The number of protons stored in the ring varies from cycle to cycle between
\SIrange{1.0e10}{2.0e10} and was measured by a beam current transformer.
...
These results show that ready to be used at HESR, with modifications only needed
to renew specific components of the target setup and the buildup of new pipe line between the
target chamber and the forward chamber.

\section{Acknowledgement}
\label{sec:acknowledgement}
We would like to thank the operating crews of the COSY group for the stable beam.
