...
The instrinsic resolution of the recoil sensor from $\alpha$ source
calibration (about \SI{20}{\keV} at \SI{5}{\MeV}) is much smaller than the extracted width of the elastic peak from the beam commissioning data (about \SI{330}{\keV} at the equivalent energy as the $\alpha$ source).
...
The accuracy of the combined fit deteriates when the elastic peak is close to
the MIP peak ($\less$ \SI{1}{\MeV}), and both peaks are embeded among the large low-energy background.
In this case, it's hard to estimate the parameters of each background component separately and a large error exists in determining the fraction of each component.
A pre-selection of the elastic scattering events is needed to get a cleaner
energy spectrum for the fit.
