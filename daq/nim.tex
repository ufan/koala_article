...
Most background events can already be supressed by requiring that the forward detector has a valid
hit from the beam particle.
However, the more accurate selection of elastic scattering events is achieved by using the TOF-E relation.
%\textit{i.e.}, the QDC amplitudes of both forward detector are larger than the lower threshold of \num{1000}.

% Elastic selection based on TOF-E
Due to the small variation of the flight time of the scattering beam particle,
TOF of the recoil proton is approximated by the difference between the hit time of the recoil detector and the forward detector.
The typical TOF-E spectrum from a single strip is shown in the inner subplot of Fig. \ref{fig:tof-e}. 
Three characteristics, which are common for all strips, are recognized in the raw spectrum:
1) a wide elastic peak ($\sim$\SI{0.9}{\MeV} on the example spectrum);
2) an upper band, which follows the same TOF-E relation as proton;
3) a lower horizontal band, which matches the time-walk corrected relation of
events with the same TOF and different energies.
The upper and lower bands merge with the elastic peak, and they are both generated by the elastically-recoiled protons.
The upper band events are from the interaction of the beam particle with the residual gas
in the beam pipe;
the lower band events are from the reocil protons hitting the edge of the
strip and thus depositing partial energy in the sensor.
\begin{figure}[h!]
  \centering
...
A data-deduced TOF-E relation is determined by profiling the central region
of the overall TOF-E spectrum along the E-axis in a step size of \SI{10}{\keV}.
...
% The TOF resolution of the recoil detector is better than dominated by the 
The obtained TOF-E relation combining with a $[-5\sigma_{TOF}, 5\sigma_{TOF}]$
window is finally applied to filter out the elastic scattering events on each strip.
...
Again, the extended binned Maximum-Likelihood fit using a combined model of the
Coulomb elastic scattering formula and the Crystal-Ball function is used to extraction of the yield of elastic events from the target body is carried out
using a combined model of a Coulomb elastic scattering formula and the Crystal-Ball function.
The fit results are shown for the three example spectra in Fig. \ref{fig:cut}.
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.45\textwidth]{./tofe_cut_comparison.png}
  \caption{Energy spectra (black) of three strips at small recoil angle on Si1 (from
...
\begin{figure}[h!]
  \centering
  \includegraphics[width=0.48\textwidth]{./e_vs_alpha_combined.png}
  \caption{
...
% \begin{figure}[h!]
%   \centering
%   \includegraphics[width=0.48\textwidth]{./fwd_coverage_ratio.png}
%   \caption{
%     Comparison of measured (red circle) and calculated (blue triangle) recoil energy with respect to the strip position along z-axis (i.e. beam direction) at beam momentum \SI{2.2}{\momentum}.}
