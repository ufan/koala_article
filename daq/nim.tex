...
KOALA is being commissioned with the proton beam from COSY \cite{COSY} to measure $pp$ elastic scattering in the beam momentum range from \SIrange{1.5}{3.4}{\momentum}.
...
The cluster beam profile was measured by using a probing rod when no accelerator beam was circulating inside COSY.
A typical thickness of $\sim\SI{1.5}{\mm}$ (FWHM) at the interaction point (IP) was determined.
...
elastic scattering and measures its total kinetic energy.
...
The sensors have different thicknesses: \SI{1}{\mm} for Si1
...
MSCF16 integrates a shaping amplifier and a leading edge discriminator in the same module.
Both amplitude and timing signals are extracted from MSCF16 for the energy and time measurement.
...
tube cold head \cite{pt30} and two heating resistors, which are thermally coupled and mounted on the detector cold plate.
A study of the performance of the recoil sensors using $\alpha$ sources in the
laboratory showed that the working temperature with the best energy resolution is
\SI{125}{\kelvin}.
...
This corresponds to an angular acceptance of the scattering angle from
...
installed inside a light-tight case on the air side of the flange as shown in Fig. \ref{fig:forward_module} (c).
\begin{figure}[htbp]
  \centering
  ...
  \caption{(a) The CAD model of the assembly of a forward detector module; (b) The
    aluminum tube is fixed on the vacuum side of the flange; (c) One forward detector module after complete assembly.}
...
\label{sec:beam}
Typically there are $10^{10}$ protons stored in COSY, which has a circumference of
\SI{184}{\meter}.
