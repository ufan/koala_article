...
between the recoil proton and the scattering proton is utilized for this selection.
...
% Most background events are can be supressed by requiring that both
% the recoil detector and the forward detector are triggered and the forward detector has a valid
...
% Due to the small flight-time variation of the forward scattering proton,
...
A typical TOF-E spectrum on a single strip is shown in the inner subplot of Fig. \ref{fig:tof-e}. 
\begin{figure}[h!]
  \centering
  % \includegraphics[width=0.35\textwidth]{./tofe_tofsigma.png}
  ...
  \label{fig:tof-e}
\end{figure}
Three features are recognized in the raw spectrum:
1) a wide elastic peak ($\sim$\SI{0.9}{\MeV} in example strip);
2) an upper band spreading from the recoil trigger threshold value around
\SI{50}{\keV} to the elastic peak;
3) a lower band horizontal band which merges with the elastic peak. 
These are the common features found in all strips which are covered by the forward detector.
...
This selection effectively filter out all the full elastic events,
...
The results of this fit for the three strips of Si1 are shown in Fig. \ref{fig:cut}.
\begin{figure}[h!]
  \centering
