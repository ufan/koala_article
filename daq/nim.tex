...
\hypersetup{
 pdfauthor={Yong Zhou},
 pdftitle={The KOALA experiment for (anti)proton-proton elastic scattering},
 pdfkeywords={},
 pdfsubject={},
...
Recent experiments like ANKE \footnote{D. Mchedlishvili et al., Phys. Lett. B 755, 92 (2016)}, EDDA \footnote{R.A. Arndt et al., Phys. Rev. C 76, 025209 (2007)} have filled the gap in the proton-proton elastic scattering database above 1 GeV in laboratory frame.
...
\label{sec:org34cf55b}
KOALA is installed at the previous ANKE segment on the COSY ring.
The full setup of KOALA consists of three arms as shown in Fig. \ref{fig:orgbabe6d2}: the hydrogen cluster target, the recoil arm and the forward arm.
...
\caption{\label{fig:orgbabe6d2}
3D visualization of the KOALA setup at COSY}
\end{figure}
...
The hydrogen cluster target is upgraded from the old ANKE target \footnote{A.Khoukaz et al., Eur. Phys. J. D 5, 275 (1999)}.
...
\label{sec:org68488aa}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./recoil_schematic.png}
\caption{\label{fig:org9ed7981}
a) A schematic view of the recoil detector; b) Layout of the four sensors}
\end{figure}
...
and the timing filter amplifier (MSCF16), both from Mesytec \footnote{\url{https://www.mesytec.com/}}. 
...
More detailed information about the recoil detector can be found in \footnote{Hu, Q., Bechstedt, U., Gillitzer, A. et al. Eur. Phys. J. A 50, 156 (2014)\label{org8c63bfc}}.

\subsection{Energy calibration}
\label{sec:orgd1d42e6}
...
\(\alpha\) sources \(^{239}Pu\), \(^{244}Cm\), \(^{241}Am\) are used for the energy calibration, with main decay energies of 5156.59 keV, 5804.83 keV, 5485.56 keV \footnote{E. Browne, J. K. Tuli Nuclear Data Sheets 122, 205 (2014)} respectively.
...
The thickness of the dead layer has already been measured in the laboratory using \(\gamma\) rays \textsuperscript{\ref{org8c63bfc}}, see Tab. \ref{tab:orgc33252e}.
...
\begin{table}[htbp]
\caption{\label{tab:orgc33252e}
Energy deposits in the dead layer for the \(\alpha\) sources used in the calibration.}
\centering
...
Thus, the separation of the energy peaks is much smaller at large recoil angle than at small recoil angle, as shown in Fig. \ref{fig:orge5b6429}.
The minor peaks can't be recognized in Fig. \ref{fig:orge5b6429} (a), while they are clearly separated in Fig. \ref{fig:orge5b6429} (b).
...
\caption{\label{fig:orge5b6429}
Energy spectrum of \(\alpha\) sources of two channels at different recoil angles: (a) small recoil angle; (b) large recoil angle}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./linearity.png}
\caption{\label{fig:orgcbd2bcd}
Electronic linearity of a typical recoil detector channel}
\end{figure}
...
A typical example is shown in Fig. \ref{fig:orgcbd2bcd}. 
Thus, the systematic error of this indirect method of energy calibration is very small.

\subsection{Time-walk correction}
\label{sec:orgea9c877}
\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./timewalk.png}
\caption{\label{fig:org2a559ec}
Typical result from the time-walk calilbration.}
\end{figure}
...
By scanning the pulser over a wide range of amplitudes, the time-walk effect is revealed as shown in Fig. \ref{fig:org2a559ec}.
...
These 4 pairs are installed symmetrically around the beam pipe center as shown in Fig. \ref{fig:orgbabe6d2}.
...
Plastic scintillator BC-408 \footnote{\url{https://www.crystals.saint-gobain.com}} is selected as the sensor material due to its good timing performance (0.9 ns rise time).
...
The scintillator is coupled to a PMT (Hamamastu H6900 \footnote{\url{https://www.hamamatsu.com}}) with a tapered light guide and a silicone pad.
...
\item a thin aluminum tube with thickness of \(100 \mu m\) is used as the light shield of the scintillator, the tube is welded on the flange (Fig. \ref{fig:orgdd0f7c4} (a);
\item two small holes are opened on the side of the aluminum tube to speed up vacuum pumping.
\end{enumerate}
Components like silicone pad, PMT, PMT base are installed in a light-tight box on the other side of the flange as shown in Fig. \ref{fig:orgdd0f7c4} (b).

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./forward_module.png}
\caption{\label{fig:orgdd0f7c4}
a) The aluminum tube is welded on the inside of the flange; b) One forward detector module after complete assembly}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./forward_mip.pdf}
\caption{\label{fig:org1e6ea6c}
An example of the cosmic ray energy spectrum obtained by a forward detector module prototype}
\end{figure}
...
A typical MIP spectrum obtained by the forward detector module using cosmic ray is shown in Fig. \ref{fig:org1e6ea6c}. 
...
For the recoil detector, the amplitude signal from MSCF16 is digitized by a peak-sensing ADC called MADC-32 \footnote{mesytec GmbH \& Co. KG, MADC-32 User Guide}.
MADC-32 has a 13-bit dynamic range with 6.4 \(\mu s\) conversion time.
For the forward detector, the pulses from PMT are directly fed into a QDC called MQDC-32 \footnote{mesytec GmbH \& Co. KG, MQDC-32 User Guide} for charge measurement.
MQDC-32 has a dynamic range of 500 pC and it uses a 12-bit ADC for digitization with 250 ns conversion time.
The timing information from both the recoil and forward detectors are recorded by the TDC module called MTDC-32 \footnote{mesytec GmbH \& Co. KG, MTDC-32 User Guide} using a conventional Start-Stop method.
MTDC-32 has a minimum resolution of 5 ps.
A multi-channel scalar called SIS3820 \footnote{\url{https://www.struck.de/}\label{org0f6a716}} is also integrated to measure the following key count rates: 1) count rates of all the four arms of the forward detector for 
...
The VME controller is SIS3100 from Struck Innovative \textsuperscript{\ref{org0f6a716}}.
...
The trigger of the DAQ system is a common OR of the sub-detectors, as shown in Fig. \ref{fig:orgbf1b662}.
...
\caption{\label{fig:orgbf1b662}
Trigger Logic of the KOALA DAQ.}
\end{figure}
...
module is added to the trigger logic as a VETO as shown in Fig. \ref{fig:orgbf1b662}.
...
An alternate option is to introduce a fixed-width mask signal into the trigger logic as VETO, as show in Fig. \ref{fig:orgbf1b662}.
...
\caption{\label{fig:org6511379}
Design and deployment of KoalaEms.}
\end{figure}

A dedicated DAQ software called KoalaEms is also developed for KOALA.
KoalaEms is a fork of the EMS software \footnote{K. H. Watzlawik et al. IEEE Transactions on Nuclear Science 43 (1996): 44}, which is a highly flexible DAQ software framework developed for various experiments previously conducted at COSY.
...
The design of KoalaEms and the topology of deployment are shown in Fig. \ref{fig:org6511379}.
...
Thus, all the square blocks in Fig. \ref{fig:org6511379} can be hosted in different PCs and new consumer host to the data stream can be integrated when needed.


\section{Software framework}
\label{sec:orga0c0cef}
...
It is built upon the FairRoot\footnote{\url{https://github.com/FairRootGroup/FairRoot}} framework, which implements a simulation environment based on VMC \footnote{I Hřivnáčová 2008 J. Phys.: Conf. Ser. 119 032025} library and an analysis environment based on ROOT's task concept.
The components stack of KoalaSoft is shown in Fig. \ref{fig:org05ce335}.

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./koalasoft_components.png}
\caption{\label{fig:org05ce335}
Components of KoalaSoft}
\end{figure}
...
\label{sec:org9b521a7}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./tof_e_cut.png}
\caption{\label{fig:org98cadbd}
...
\caption{\label{fig:org049efbe}
Energy spectrum of Si1\_16 before (black) and after (blue) TOF-E cut.}
\end{figure}

\begin{figure}[htbp]
\centering
\includegraphics[width=0.9\textwidth]{./calc_vs_measured_combined.png}
\caption{\label{fig:org47d8c50}
...
A typical result of TOF-E spectrum from the recoil protons is shown in Fig. \ref{fig:org98cadbd}. 
...
To select the elastic events, the fitting result of the TOF-E major band is moved up and down to form a cut region as shown in the pink curves in Fig. \ref{fig:org98cadbd}.
A typical result after applying the TOF-E cut is shown in Fig. \ref{fig:org049efbe}.
...
Fig. \ref{fig:org47d8c50} shows the comparison between the measured energy peak and the expected energy of recoil proton from elastic scattering at 2.6 GeV/c.
A limit is observed around \(250 keV\), which corresponds to |t| approximately 0.0005 \((GeV/c)^2\).

\section{Conclusion}
\label{sec:org3f8a87b}
