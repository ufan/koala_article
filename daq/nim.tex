...
smaller thickness of the cluster along the beam axis.
...
Thus, the recoil detector is required to measure the energy deposit up to \SI{60}{\MeV}.
...
Typical timing resolutions are about \SI{140}{\pico\second}.
...
\cite{recoil_article} and the energy loss of $\alpha$ particles can be estimated with LISE++ program \cite{LISE}.
...
The minor decay modes can not be recognized with a small gain setting (the black
curve in Fig. \ref{fig:alpha_spectrum}), while they are clearly seen with a large gain
setting (the blue curve in Fig. \ref{fig:alpha_spectrum}).
...
\begin{figure}[ht!]
  ...
  \caption{Energy spectra of \(\alpha\) sources for two channels at small (blue)
    and large (black) recoil angles. The ADC channel are already converted the
    energy value using the calibration parameters. The The first $\alpha$ peak in each
    spectrum is fitted with a Gaussian. }
...
between the measurement data points and the linear term have small dispersions
($\sigma=2.9$ for the beam gain setting, $\sigma=12.1$ for the common gain setting).
...
\begin{figure}[htb!]
  ...
  \caption{The results of a quadratic fit to the linearity curves of a recoil
    ...
    points and the linear term of the quadratic function.}
...
\label{sec:beam}
Typical number of stored protons inside COSY is about \SI[per-mode=reciprocal]{e10}.
The storage time in a beam cycle is about \SI{300}{\second}, as shown in Fig. \ref{fig:beam}.
...
The experiment is only carried out in the storage cycle, which is achieved by two
