...
\begin{frontmatter}	
  ...
  \begin{abstract}
    ...
    % goals of nuclear and hadron physics.
    ...
    transfer squared $|t|$ from \SIrange{0.0008}{0.1}{\tmom}.
    These data are essential input for the PANDA experiment to reach the desired
    absolute precision for the integrated luminosity determination.
    ...
    The KOALA experiment is based on fixed target kinematics and uses an internal hydrogen cluster jet target.
    The wide range of $|t|$ is achieved by measuring the total kinetic energy of the recoil
    protons near \SI{90}{\degree}.
    The recoil detector consists of silicon and germanium single-sided strip
    sensors with an energy resolution of approximately \SI{20}{\keV} and
    \SI{30}{\keV} (FWHM), respectively.
...
% Solid-state sensors (especially germanium) need a low and stable operating
...
% The actual coverage can to be extracted in the data analysis, see Sec. \ref{sec:result}.
...
% MQDC-32 has a dynamic range of \SI{500}{\pico\coulomb} with 12-bit resolution and \SI{250}{\nano\second} conversion time.
...
% MTDC-32 modules are configured to work in the Start-Stop mode, with the DAQ trigger signal serving as the common start.
A multi-channel scalar SIS3820 \cite{sis} is also integrated to record some key count rates.
% : 1) count rates of the four pairs of the forward detector for beam position monitoring; 2) count rates of the overlapping regions of the recoil detector for asymmetry correction; 3) count rates of the input trigger for DAQ efficiency correction.
...
% The achieved precision of the calibration is about \SI{0.12}{\percent}.
...
\begin{figure}[h!]
  ...
  \caption{Histograms of the strip-wise difference between the calculated position from the
...
% Comparing the conventional experiment, the identification of the impinging strip of
...
% The characteristics of the background shape also show up with two explicit components: 1) a peak which equals MIP's energy loss in the recoil sensors; 2) a smooth decay with a long tail.
...
% The deterioration of the resolution near the edge of the spectrum is mainly
...
% LocalWords:  COSY TOF HESR ANKE IKP preamplifiers MPR MSCF photomultiplier
% LocalWords:  MIP DAQ VME
