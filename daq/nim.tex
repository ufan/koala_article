...
trigger logic as a \texttt{VETO}, as shown in Fig. \ref{fig:trigger_logic} (b).

The need for event synchronization arises naturally when using the asynchronous
readout mode.
The event synchronization is imposed at the hardware-level by adding a
fixed-width mask gate into the trigger logic as \texttt{VETO}, see Fig.
\ref{fig:trigger_logic} (b).
